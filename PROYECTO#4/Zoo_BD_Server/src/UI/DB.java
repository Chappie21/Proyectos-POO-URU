...
public class DB {
	...
	//BUSQUEDA POR NOMBRE
	public String BuscarN(String nom) {
		try {
			this.st = this.con.createStatement();
			this.rs = this.st.executeQuery("SELECT*FROM Animales");/*EJECUTA LA SENTENCIA SQL INDICADA POR EL STRING "sentencia"*/ 
			while(rs.next()) {
				/*(FORMA NO2 PARA EVITAR ERRORES DE ESCRITURA)SI ALGUNO DE LOS NOMBRE COINCIDE CON EL INGRESADO POR EL USUARIO EN EL CHAT
				 * RETORNA EL ANIMAL PEDIDO, DE LO CONTRARIO RETORNA NULL
				 * DEBEMOS TOMAR EN CUENTRA QUE EL USUARIO PUEDE ESCRIBIR
				 * POR EJEMPLO: lOKY Y AL BUSCAR EN LA BASE DE DATOS 
				 * NO LO ENCONTRARÁ DEBIDO A QUE ESTA ESCRITO DE FORMA
				 * DIFERENTE (SI USAMOS UNA SENTENCIA SQL COMO EN VECES PASADAS)
				 * DE ESTÁ MANERA NO IMPORTARA COMO
				 * LO ESCRIBA EL USUARIO, LO ENCONTRARÁ*/
				if(rs.getString("nombre").equalsIgnoreCase(nom)) {
					return "-->ID: "+rs.getString("Id_animal")+" Nombre: "+rs.getString("nombre")+", Especie :"+rs.getString("especie")
					+", Peso: "+rs.getString("peso")+", Altura: "+rs.getString("altura")
					+", Edad: "+rs.getString("edad")+", Grupo: "+rs.getString("grupo")+"\n";
				}
			}
